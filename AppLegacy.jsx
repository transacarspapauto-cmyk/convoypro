/* eslint-disable react/prop-types, react/no-unescaped-entities, no-inner-declarations */
...
/* ═══════ DATA — voir hooks/useMissions.js ═══════ */
...
/* ═══════════════════════════════════════════════════
   ÉTAT DES LIEUX — Logique (initEDLData importé depuis hooks/useEDL.js)
═══════════════════════════════════════════════════ */
...
function Step8({d,od,m,userName}){
  return (
    <div className="fade">
      <Card style={{ marginBottom:12, background:C.bgDeep }}>
        <div style={{ fontSize:13, fontWeight:600, color:C.textPrimary, marginBottom:4 }}>Mission {m?.ref}</div>
...
function MobileApp({ showPrice = false, missions = [], userId, userName = "" }) {
  ...
  const { data, setData, saveEDL } = useEDL(selM?.id, userId);
  ...
  if(inEDL && selM){
    ...
    function tryNext(){if(canGo){setErrors([]);saveEDL(data);setStep(s=>Math.min(9,s+1));scrollRef.current?.scrollTo({top:0,behavior:"smooth"});}else{setErrors(errs);setShaking(true);setTimeout(()=>setShaking(false),400);}}
